...
class SparePart(Document):
	...
	def on_update(self):
		...
		if self.stock_quantity is not None and self.stock_quantity <= threshold:
			frappe.msgprint(f"Stock for {self.part_name} is low ({self.stock_quantity} units left). Consider reordering.")
