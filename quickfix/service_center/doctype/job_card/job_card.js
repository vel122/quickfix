frappe.ui.form.on("Job Card", {
	...
	refresh(frm) {
		...
		if (frappe.boot.quickfix_shop_name) {
			frm.dashboard.set_headline(__("Shop: {0}", [frappe.boot.quickfix_shop_name]));
		}
		...
		if (frm.doc.docstatus === 1) {
			frm.add_custom_button("Transfer Technician", function () {
				frappe.prompt(
					...
					function (values) {
						frappe.confirm(
							`Are you sure you want to transfer this job to ${values.new_technician}?`,
							function () {
								if (frm.doc.assigned_technician === values.new_technician) {
									frappe.msgprint(
										"This technician is already assigned to this job."
									);
									return;
								}
								frm.call({
		...
		frappe.db
			...
			.then((res) => {
				if (res.message.specialization !== frm.doc.device_type) {
					frappe.warn(
						"Selected technician does not specialize in the device type. Please select a different technician."
					);
